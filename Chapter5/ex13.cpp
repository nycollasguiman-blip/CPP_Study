...
vector<int> randon_vec(int length){
    // Gera vetor com números aleatórios com tamanho 'length'
    vector<int> rand_numbers;
    int number;
    for (int i = 0; rand_numbers.size() < length; ++i){
        number = randint(9);
        if (number_in(number, rand_numbers) == -1){
            rand_numbers.push_back(number);
        } 
    }
    return rand_numbers;
}
...
void game(){
    ...
    while(true){
        try{
            ...
            int seed;
            if (answer == 's'){
                cout << "Digite uma seed: \n";
                cin >> seed;
                srand(seed);
                pc_numbers = randon_vec(pc_numbers.size());
                oxen = 0;
            }
            else
                break;
